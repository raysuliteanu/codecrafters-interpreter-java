...
public class Scanner {
    Tuple thisOrThat(final PeekableIterator chars, final Character match, final Lexemes combo, final Lexemes single, final long offset) {
            ...
            for (var c : chars) {
                ...
                TokenBuilder token = switch (c) {
                    ...
                    case Character ch when Character.isDigit(ch) -> {
                        ...
                        StringBuilder sb = new StringBuilder();
                        sb.append(ch);
                        while (chars.hasNext() && matches(chars.peek(), (t) -> (t >= '0' && t <= '9') || t == '.')) {
                            Character next = chars.next();
                            trace("adding '" + next + "'");
                            sb.append(next);
                            ++offset;
